...
BOOL CComFunc::IsFileExist(const CString& csFile)
{
	DWORD dwAttrib = GetFileAttributes((LPSTR)(LPCSTR)csFile);
	return INVALID_FILE_ATTRIBUTES != dwAttrib && 0 == (dwAttrib & FILE_ATTRIBUTE_DIRECTORY);
}
// 判断文件夹是否存在
BOOL CComFunc::IsDirExist(const CString & csDir)
{
	DWORD dwAttrib = GetFileAttributes((LPSTR)(LPCSTR)csDir);
	return INVALID_FILE_ATTRIBUTES != dwAttrib && 0 != (dwAttrib & FILE_ATTRIBUTE_DIRECTORY);
}
