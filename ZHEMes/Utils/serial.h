...
public:
	...
	CSerial &operator<<(const CString &tStrIn)
	{
		BYTE str[256]; 
		UINT uiLen;
		ComTool::WCharToMByte((LPCTSTR)tStrIn,(LPSTR)str,sizeof(str)/sizeof(str[0]));
		uiLen=(UINT)strlen((char*)str)+1;
		IncreaseBuffer(uiLen);
		memcpy(m_pBuff+m_dwLen,str, uiLen);
		m_dwLen += uiLen;
		return *this;
	}
	CSerial & operator>>(CString &tStrOut)
	{
		...
		ComTool::MByteToWChar((LPSTR)(m_pBuff+m_dwStart),wStr,sizeof(wStr)/sizeof(wStr[0]));
		tStrOut.Format(_T("%s"),wStr);
		DecreaseBuffer((UINT)strlen((LPSTR)m_pBuff+m_dwStart) + 1);
		return *this;
	}
#else
	CSerial &operator<<(const CString &tStrIn)
